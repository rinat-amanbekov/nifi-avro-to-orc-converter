...
public class NiFiOrcUtils {
  ...
  public static Object convertToORCObject(TypeInfo typeInfo, Object o) {
    if (o != null) {
      ...
      if (o instanceof Long) {
        if (typeInfo != null && typeInfo.equals(TypeInfoFactory
            .getPrimitiveTypeInfoFromPrimitiveWritable(TimestampWritable.class))) {
          return new TimestampWritable(new Timestamp((Long) o));
        }
  ...
  private static TypeInfo getLogicalTypeInfo(Schema schema1){
    String type = schema1.getProp("logicalType");
    schema1.getProp("testProp");
    schema1.getObjectProps();
    switch (type){
      case "decimal":
        int precision = schema1.getObjectProp("precision") != null
            ? Integer.valueOf(schema1.getObjectProp("precision").toString())
            : 10;
        int scale = schema1.getObjectProp("scale") != null
            ? Integer.valueOf(schema1.getObjectProp("scale").toString())
            : 2;
        return new DecimalTypeInfo(precision, scale);
      case "timestamp-millis":
        return TypeInfoFactory.getPrimitiveTypeInfo("timestamp");
    }
